...
- Admin LLM chat supports two record-context modes: legacy eager Qdrant enrichment and `RECORD_AGENT`, a bounded records-agent path that lets the model request one `search_records` lookup when needed. Both modes reuse backend visibility/recency filtering before any record chunks are sent to the LLM.
...
- Backend LLM helpers use the Market Pulse chat endpoint for admin-only chat, record label generation, chat summarization, chat-to-record creation, and market-news summaries. Built-in prompt text and Java-side output token budgets are centralized under `com.yipeng.recorder.prompt`; frontend code should not set chat token budgets.
...
- Daily market-news summaries use yfinance news plus the LLM. Market Pulse reads default symbols from `resources/symbols/news_symbols.txt`, bounds news input size through `NEWS_LLM_*` env values, passes through the backend-owned prompt/token budget, cleans reasoning/markdown/unfinished output from model output, and falls back to article text when the LLM fails. Backend creates public records in chunks controlled by `CronService.MARKET_NEWS_SUMMARY_SYMBOLS_PER_RECORD`, with date/market labels and uppercase symbol labels.
