...
- Semantic search uses Market Pulse `/qdrant` APIs. Backend builds an embedding string from record title, labels, and content, then asynchronously upserts/deletes vectors after record changes.
...
- Admin LLM chat can request related-record context. Backend retrieves Qdrant chunk hits for the latest user message, rechecks record visibility and dates in MySQL, keeps up to 5 recent/relevant chunks, and injects them as background context with source title, record id, created date, and modified date. The LLM is instructed to synthesize from chunks rather than repeat record content.
...
- Backend LLM helpers use the Market Pulse chat endpoint for admin-only chat, record label generation, chat summarization, chat-to-record creation, and market-news summaries.
