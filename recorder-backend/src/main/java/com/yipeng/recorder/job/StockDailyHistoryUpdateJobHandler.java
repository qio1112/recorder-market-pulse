...
@Component
public class StockDailyHistoryUpdateJobHandler implements JobHandler {

    private final CronService cronService;

    public StockDailyHistoryUpdateJobHandler(CronService cronService) {
        this.cronService = cronService;
    }

    @Override
    public String jobType() {
        return JobType.STOCK_DAILY_HISTORY_UPDATE;
    }

    @Override
    public JobResult run(JobContext context) {
        String timeName = getStringParam(context, "timeName", "manual");
        CronService.StockJobResult result = cronService.updateStockDailyHistory(timeName);
